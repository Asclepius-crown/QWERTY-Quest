    ...
    <div className="min-h-screen bg-base-dark text-base-content font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 relative -mt-20">
            <div className="flex flex-col md:flex-row items-end md:items-center gap-6 pb-6 border-b border-base-content/10">
                ...
                <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl bg-base-dark border-4 border-base-dark relative z-10 shadow-2xl">
                    <div className="w-full h-full rounded-xl bg-gradient-to-br from-white/10 to-transparent flex items-center justify-center border border-base-content/10">
                        <User className="w-16 h-16 text-base-muted" />
                    </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
            ...
            >
                <h3 className="text-base-muted text-xs font-bold uppercase tracking-widest mb-4">Skill Stats</h3>
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-base-content/5 rounded-xl border border-base-content/5">
                        <div className="text-2xl font-mono font-bold text-base-content">{user?.stats?.avgWPM || 0}</div>
                        <div className="text-xs text-base-muted">Avg WPM</div>
                    </div>
                    <div className="p-3 bg-base-content/5 rounded-xl border border-base-content/5">
                        <div className="text-2xl font-mono font-bold text-green-400">{user?.stats?.bestWPM || 0}</div>
                        <div className="text-xs text-base-muted">Best WPM</div>
                    </div>
                    <div className="p-3 bg-base-content/5 rounded-xl border border-base-content/5">
                    ...
                    <div className="p-3 bg-base-content/5 rounded-xl border border-base-content/5">
                        ...
                        <div className="text-xs text-base-muted">Wins</div>
                    </div>
                </div>
            </motion.div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
                <h3 className="text-xl font-bold text-base-content mb-2">Recent Activity</h3>
                {recentMatches.map((match, idx) => (
                    <motion.div 
                        ...
                        className="glass-card p-4 rounded-xl border border-base-content/5 flex items-center justify-between hover:bg-base-content/5 transition-colors group"
                    >
                        <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold ${match.result === 'Win' ? 'bg-green-500/20 text-green-400' : 'bg-gray-700/50 text-base-muted'}`}>
                                {match.result === 'Win' ? 'W' : match.result}
                            </div>
                            <div>
                                <div className="text-base-content font-bold">{match.mode} Match</div>
                                <div className="text-xs text-base-muted">{match.date}</div>
                            </div>
                        </div>
                        
                        <div className="flex items-center gap-8">
                            <div className="text-right">
                                <div className="text-sm text-base-muted uppercase text-[10px]">Speed</div>
                                <div className="text-xl font-mono font-bold text-base-content">{match.wpm}</div>
                            </div>
                            <div className="text-right">
                                <div className="text-sm text-base-muted uppercase text-[10px]">Acc</div>
                                <div className="text-xl font-mono font-bold text-green-400">{match.acc}%</div>
                            </div>
                            <button className="p-2 hover:bg-base-content/10 rounded-lg text-base-muted hover:text-base-content transition-colors">
