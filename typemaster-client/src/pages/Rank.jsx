...
const ranks = [
  ...
  { id: 'silver', name: 'Silver', color: 'text-gray-300', border: 'border-gray-400', bg: 'bg-gray-700/20', shadow: 'shadow-gray-500/50', minXp: 1000, reward: 'Silver Frame' },
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="relative z-10">
                ...
                <h1 className={`text-5xl md:text-7xl font-black uppercase tracking-tighter ${currentRank.color} drop-shadow-lg`}>
                    {currentRank.name} <span className="text-white text-4xl align-top">II</span>
                </h1>
                <p className="text-xl text-gray-400 mt-2 font-mono">Global Rank #12,430 • Top 15%</p>
            </div>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="glass-card p-8 rounded-2xl border border-white/5 bg-gradient-to-r from-base-navy/50 to-transparent">
                    <div className="flex justify-between items-end mb-2">
                        <div>
                            <span className="text-sm text-gray-400 uppercase tracking-widest font-bold">Current Progression</span>
                            <div className="text-2xl font-mono font-bold text-white mt-1">
                                {currentXp} <span className="text-gray-500 text-lg">/ {rankEnd} XP</span>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        {ranks.map((rank, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`p-2 rounded-full ${rank.bg} ${isUnlocked ? rank.color : 'text-gray-500'}`}>
                                        {isUnlocked ? <Medal className="w-6 h-6" /> : <Lock className="w-6 h-6" />}
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex justify-between items-center">
                                            <span className={`font-bold ${isUnlocked ? 'text-white' : 'text-gray-500'}`}>{rank.name}</span>
                                            <span className="text-xs font-mono text-gray-500">{rank.minXp} XP</span>
                                        </div>
            ...
            <div className="space-y-8">
                ...
                <div className="glass-card p-6 rounded-2xl border border-primary/20 bg-gradient-to-b from-primary/10 to-transparent relative overflow-hidden">
                    ...
                    <div className="space-y-4 relative z-10">
                        ...
                        <div className="space-y-2">
                            <div className="text-xs text-gray-400 font-bold uppercase">Season Reward</div>
                            <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-white/10">
                                <Crown className="w-5 h-5 text-yellow-400" />
                                <div>
                                    <div className="text-sm font-bold text-white">Genesis Emblem</div>
                                    <div className="text-xs text-gray-500">Reach Gold Tier</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="glass-card p-6 rounded-2xl border border-white/5">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                        <Shield className="w-5 h-5 text-gray-400" /> History
                    </h3>
